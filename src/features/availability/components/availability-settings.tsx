  ...
  return (
    <section className="mb-7 rounded-lg border border-border bg-white p-5 shadow-soft">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold">Configuracao de agenda</h2>
      ...
      <form action={rulesAction} className="grid gap-3">
        {weekDays.map((day) => {
      ...
      <div className="mt-6 border-t border-border pt-5">
        <h3 className="text-base font-semibold">Bloqueios por data</h3>
        <form action={blockAction} className="mt-3 grid gap-3 md:grid-cols-[180px_1fr_auto]">
          <Field label="Data">
        ...
        <div className="mt-4 grid gap-2">
          {dateBlocks.length ? (
            dateBlocks.map((block) => (
              <div
                key={block.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-md bg-muted px-3 py-2 text-sm"
              >
                <span>
